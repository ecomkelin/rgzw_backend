const CourseSV = require('./service');
...
class CourseCT {
  ...
  detail = async (req, res) => {
    try {
      ...
      const { item } = await CourseSV.detail(req.payload, id, options);

      return res.status(200).json(ApiResponse.success({ data: { item } }));
    } catch (e) {
      console.error("CourseCT detail error: ", e);
      return res.json(ApiResponse.error(e));
    }
  };
  ...
  add = async (req, res) => {
    try {
      const { item } = await CourseSV.add(req.payload, req.validData);

      return res.status(200).json(ApiResponse.success({ data: { item } }));
    } catch (e) {
      console.error("CourseCT add error: ", e);
  ...
  edit = async (req, res) => {
    try {
      ...
      const { item } = await CourseSV.edit(req.payload, id, doc);
      return res.status(200).json(ApiResponse.success({ data: { item } }));
    } catch (e) {
      console.error("CourseCT edit error: ", e);
      return res.status(500).json(ApiResponse.error(e));
    }
  };
  ...
  remove = async (req, res) => {
    try {
      const id = req.validData?.id;
      const { item } = await CourseSV.remove(req.payload, id);
      return res.status(200).json(ApiResponse.success({ data: { item } }));
    } catch (e) {
      console.error("CourseCT remove error: ", e);
      return res.status(500).json(ApiResponse.error(e));
    }
  };
}

module.exports = new CourseCT();