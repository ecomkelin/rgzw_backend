...
- **账户模型**: 一个 `Account` 可对应多个 `User`(员工) / 多个 `Student`(学生)
  ...
  - `Student` 账户下可有多个学生
- **双令牌机制**: 短时效 `accessToken`（默认 5 分钟，由 `ACCESS_TTL_M` 控制）+ 长时效 `refreshToken`（HttpOnly Cookie，**默认 7 天**，由环境变量 `REFRESH_TTL_D` 控制；本仓库 `.env` 当前设为 30）
...
- 刷新令牌(Refresh Token): 存储在HttpOnly Cookie中，长期有效（**默认7天**，由环境变量 `REFRESH_TTL_D` 控制；本仓库 `.env` 当前设为 30），用于获取新的访问令牌
